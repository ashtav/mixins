...
class Forms {
  ...
  /// ```
  static void fill(
      Map<String, TextEditingController> forms, Map<String, String?> data,
      ...
      for (var e in min) {
        ...
        if (forms[key] != null && forms[key]!.text.trim().length < length) {
          keys.add({
            'key': key,
            'type': 'min',
            'message': '$key must be at least $length characters'
          });
        }
      }
      ...
      for (var e in max) {
        ...
        if (forms[key] != null && forms[key]!.text.trim().length > length) {
          keys.add({
            'key': key,
            'type': 'max',
            'message': '$key must be at most $length characters'
          });
        }
      }
        ...
        children: [
          ...
          Container(
            decoration: BoxDecoration(
                border: Br.all(Colors.black12), borderRadius: Br.radius(2)),
            child: ClipRRect(
              ...
              child: Column(
                children: List.generate(children.length, (i) {
                  ...
                  if (child is Input) {
                    ...
                    child = child.copyWith(
                        margin: Ei.none, border: border, borderRadius: zero);
                  ...
                  return Container(
                    decoration:
                        BoxDecoration(border: Br.only(['t'], except: i == 0)),
                    child: child,
                  );
                }),
              ),
